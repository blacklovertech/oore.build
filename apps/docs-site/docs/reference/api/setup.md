...
Verify the one-time bootstrap token generated by [`oore setup open`](/reference/cli/oore-setup#setup-open). On success, consumes the token and returns a setup session token.
