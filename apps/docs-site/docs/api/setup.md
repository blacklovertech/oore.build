...
## Finalize Owner {#finalize-owner}

Set the owner email address directly (without OIDC verification). For OIDC-verified owner creation, use [Start Owner OIDC](#start-owner-oidc) and [Verify Owner OIDC](#verify-owner-oidc) instead.
...
| Field | Type | Required | Description |
|---|---|---|---|
| `owner_email` | `string` | Yes | Email address for the platform owner |
