...
- Added GitHub Actions release workflow for semver tags (`v*.*.*`) with per-arch macOS packaging and checksums.
...
- [x] `scripts/install.sh` installs release binaries from GitHub Releases and verifies checksums.
...
- [x] Release workflow builds and publishes macOS arm64 + x86_64 artifacts plus checksums.
